...
namespace CargoStack
{
    ...
    /// 디오라마 카메라는 꺼져 있는 동안에도 트럭을 계속 따라다녀야 전환 순간 화면이 튀지 않는다.
    /// AudioListener도 활성 카메라와 함께 전환해 공간 음향이 현재 시점을 기준으로 들리게 한다.
    /// </summary>
    public sealed class CameraDirector : MonoBehaviour
    {
        ...
        public void Frame(GameState state)
        {
            ...
            AudioListener dioramaListener = dioramaCamera.GetComponent<AudioListener>();
            if (firstPersonListener != null)
            {
                firstPersonListener.enabled = loading;
            }

            if (dioramaListener != null)
            {
                dioramaListener.enabled = !loading;
            }

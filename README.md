...
| `F` (누름 유지 후 놓기) | 들고 있는 화물을 카메라 정면으로 던지기 (1초에 최대 충전) |
...
접지 한계의 차이만큼 관성으로 밀린다. 마찰 0.02인 얼음에서 최대 횡이탈은 2.38m,
횡속도는 1.26m/s, 슬립 각은 약 24도, 롤은 약 3.3도였다. 같은 경로의 마찰을 1.0으로
올린 대조군은 횡이탈 0.03m, 횡속도 0.06m/s로 미끄러짐이 사라졌다.
...
싣고 로프 세 개를 모두 사용한 자동 주행에서도 3/8만 생존하며, 화물의 최대 상승속도는
4.65m/s였다. 로프 없이 무조건 전멸시키는 점프대가 되지 않도록 초기 2.8m 설계에서 낮췄다.

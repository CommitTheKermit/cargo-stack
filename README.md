...
집는 거리는 3.8m, 놓을 면을 찾는 거리는 6m다. 트럭 한쪽에 선 채로 짐칸 건너편 구석까지
닿으라고 잡은 값이다(그러려면 3.4m가 필요하다). 조준은 화면 중앙 레이캐스트가 하므로
...
- 트럭 한쪽에 선 채로 짐칸 건너편 구석까지 손이 닿는다 (사거리를 줄이면 트럭을 빙 돌아야 한다)
